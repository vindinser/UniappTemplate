		...
		<view class="login-form">
			<u-form ref="loginForm" :model="loginForm" :rules="rules">
				<u-form-item label="手机号码" prop="username">
					<u-input v-model="loginForm.username" placeholder="输入用户名" />
				</u-form-item>
			...
			<view class="register-btn">
				<text class="u-primary" @click="handleAgrement">找回密码</text>
				<text class="u-primary u-m-l-24 u-m-r-24">|</text>
				<text class="u-primary" @click="handleAgrement">立即注册</text>
			</view>
		</view>
...
<style lang="scss">
	.login-head {
		...
		@include custom-text($s: 24px, $w: 600, $H: 1.8);
	}
	...
	.accept-terms {
		flex: 1;
		@include custom-flex($align: flex-end, $justify: center);
		line-height: 24px;
		padding-bottom: 32px;
	}
</style>
